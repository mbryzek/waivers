...
class PdfCoService @Inject()(
  ...
  def createSignatureRequest(signature: Signature, user: User, waiver: Waiver): Future[ValidatedNec[String, String]] = {
      ...
      response.body match {
        case Right(json: JsValue) =>
          ...
          if (!success) {
            ...
            s"PDF.co API error: $errorMessage".invalidNec
          }
        case Left(error) =>
          println(s"[PDF.co API Error] Failed to create PDF: $error")
          s"PDF.co API error: $error".invalidNec
      }
    }.recover {
      case ex: Exception =>
        println(s"[PDF.co Error] Exception during PDF creation: ${ex.getMessage}")
        s"Exception during PDF creation: ${ex.getMessage}".invalidNec
    }
  }

  def addSignatureToPdf(pdfUrl: String, signatureData: String, x: Int = 450, y: Int = 200): Future[ValidatedNec[String, String]] = {
    ...
    backend.send(request).map { response =>
      ...
      case ex: Exception =>
        ...
        s"Exception during signature addition: ${ex.getMessage}".invalidNec
    }
  }
